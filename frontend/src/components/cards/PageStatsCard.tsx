...
function Metric({ label, value, sub, valueClass = "text-slate-100" }: {
  ...
  return (
    <div className="flex flex-col gap-1 min-w-0">
      <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider whitespace-nowrap">{label}</span>
      <span className={`text-2xl font-bold leading-none ${valueClass}`}>{value}</span>
      {sub && <span className="text-xs text-slate-600 whitespace-nowrap">{sub}</span>}
    </div>
  );
}
...
export function PageStatsCard({ pageStats }: { pageStats: PageStats }) {
  ...
  const metrics = [
    ...
    },
    {
      label: "Links",
      value: totalLinks,
      sub: `${pageStats.internalLinks} internal · ${pageStats.externalLinks} external`,
    },
    {
      label: "Headings",
  ...
  return (
    <div className="card card-accent-teal">
      ...
      <div className="flex items-start gap-0">
        {metrics.map((m, i) => (
          <div key={m.label} className="flex items-stretch gap-0 flex-1 min-w-0">
            {i > 0 && <div className="w-px bg-slate-700/50 mx-5 self-stretch" />}
            <Metric {...m} />
          </div>
        ))}
      </div>
